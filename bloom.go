...
func (r *redisBitSet) check(offsets []uint) (bool, error) {
	...
	resp := r.store.Eval(ctx, testScript, []string{r.key}, args)
...
func (r *redisBitSet) expire(seconds int) error {
	...
	return r.store.Expire(ctx, r.key, time.Duration(seconds)*time.Second).Err()
}
...
func (r *redisBitSet) set(offsets []uint) error {
	...
	resp := r.store.Eval(ctx, setScript, []string{r.key}, args)
	if resp.Err() == redis.Nil {
		return nil
	}
